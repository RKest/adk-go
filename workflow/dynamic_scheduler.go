...
// runChildOptions carries per-call configuration for runChild.
...
// runChild executes child once, forwards its events upstream, and
...
func (s *dynamicSubScheduler) runChild(ctx context.Context, child Node, input any, opts runChildOptions) (any, error) {
	name := child.Name()
	...
	childCtx := newChildNodeContext(s.parentCtx.WithGoCtx(ctx), childPath, runID, s)
...
func (s *dynamicSubScheduler) resolveRunID(childName, custom string) (string, error) {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	s.counters[childName]++
	return strconv.Itoa(s.counters[childName]), nil
}
