...
func TestDynamicNode_HITL_SwallowsInterrupt(t *testing.T) {
	// Child issues RequestedInput. The sub-scheduler forwards the
	// event upstream and surfaces ErrNodeInterrupted to RunNode.
	// dynamicNode.Run must NOT re-yield the error: the engine already
	// sees the pause via the forwarded event.
...
func TestDynamicNode_TerminalOutputEvent(t *testing.T) {
	...
	if last.Actions.StateDelta["output"] != 42 {
		t.Errorf("StateDelta[\"output\"] = %v, want 42 (dual-write)", last.Actions.StateDelta["output"])
	}
}
...
// failingNode yields a single error and exits.
