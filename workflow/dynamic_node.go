...
// applyDynamicDefaults sets RerunOnResume to &true when unset.
...
func (n *dynamicNode[IN, OUT]) Run(ctx agent.InvocationContext, input any) iter.Seq2[*session.Event, error] {
	return func(yield func(*session.Event, error) bool) {
		...
		orchestratorCtx := newDynamicActivationContext(parentNC, sub.parentPath, sub)

		out, err := n.fn(orchestratorCtx, typedInput, emit)
		if err != nil {
			// HITL signals from RunNode reach here as ErrNodeInterrupted.
			// The RequestedInput event was already forwarded upstream by
			// the sub-scheduler; the engine sees the pause via that event,
			// so this branch swallows the sentinel without yielding error.
...
// makeEmit wraps yield as an emit callback, surfacing ctx.Err() when
