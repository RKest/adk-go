...
func TestSubScheduler_RunChild_FreshExecution(t *testing.T) {
	...
	out, err := sub.runChild(context.Background(), child, "world", runChildOptions{})
	if err != nil {
...
func TestSubScheduler_RunChild_CustomIDInPath(t *testing.T) {
	child := newStubNode("processor", nil)
	sub := newDynamicSubScheduler(newTopLevelCtx(t), "wf", noopEmit)

	if _, err := sub.runChild(context.Background(), child, nil, runChildOptions{customRunID: "order-42"}); err != nil {
		t.Fatalf("runChild: %v", err)
	}
...
func TestSubScheduler_RunChild_HITLReturnsInterrupted(t *testing.T) {
	...
	_, err := sub.runChild(context.Background(), child, nil, runChildOptions{})
...
func TestSubScheduler_RunChild_ErrorWinsOverInterrupt(t *testing.T) {
