...
type NodeContext interface {
	...
	// WithGoCtx returns a NodeContext with the underlying context.Context
	// replaced; session, invocation metadata, and resume inputs are
	// preserved.
...
// newChildNodeContext wraps parent for a dynamic child activation.
// Child inherits resumeInputs so HITL responses reach dynamic children.
func newChildNodeContext(parent NodeContext, path, runID string, sub *dynamicSubScheduler) *nodeContext {
...
// newDynamicActivationContext wraps parent for a dynamic node's own
// activation, attaching the sub-scheduler RunNode reaches from the
// orchestrator body. runID is inherited from parent (this activation
// is not itself a sub-scheduler child).
func newDynamicActivationContext(parent NodeContext, path string, sub *dynamicSubScheduler) *nodeContext {
	var inherited map[string]any
	if p, ok := parent.(*nodeContext); ok {
		inherited = p.resumeInputs
	}
